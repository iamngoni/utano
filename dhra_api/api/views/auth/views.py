class SignInView(APIView):
    ...
    def post(self, request):
        try:
            ...
            if payload.is_valid():
                ...
                if user is not None:
                    ...
                    if not user.account.is_verified:
                        send_email_verification_otp.delay(request.user)
            ...
            return uhuru_response(num_status=500, bool_status=False)
...
class ResetPasswordView(APIView):
    ...
    def post(self, request):
        try:
            ...
            logger.error("[Auth]: User does not exist")
            return uhuru_response(num_status=404, bool_status=False)
        except Exception as e:
            logger.error(f"[Auth]: {e}")
            return uhuru_response(num_status=500, bool_status=False)
